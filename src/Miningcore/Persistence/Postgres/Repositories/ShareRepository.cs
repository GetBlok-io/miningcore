...
namespace Miningcore.Persistence.Postgres.Repositories
{
    public class ShareRepository : IShareRepository
    {
        ...
        public async Task<Share[]> ReadMinerSharesBeforeCreatedAsync(IDbConnection con, string poolId, string miner, DateTime before, bool inclusive, int pageSize)
        {
            ...
            return (await con.QueryAsync<Entities.Share>(query, new { poolId, before, pageSize, miner }))
                .Select(mapper.Map<Share>)
                .ToArray();
        }
        ...
        public Task<double?> GetShareDiffByMinerBetweenCreatedAsync(IDbConnection con, string poolId, string miner, DateTime start, DateTime end)
        {
            logger.LogInvoke(new object[] { poolId });

            const string query = "SELECT SUM(difficulty) FROM shares WHERE poolid = @poolId AND miner = @miner AND created > @start AND created < @end";
