...
namespace Miningcore.Persistence.Postgres.Repositories
{
    public class MinerRepository : IMinerRepository
    {
        ...
        {
            const string query = "INSERT INTO miner_settings(poolid, address, paymentthreshold, created, updated, donation) " +
                                 "VALUES(@poolid, @address, @paymentthreshold, now(), now(), @donation) " +
                                 "ON CONFLICT ON CONSTRAINT miner_settings_pkey DO UPDATE " +
                                 "SET paymentthreshold = @paymentthreshold, updated = now(), donation = @donation " +
                                 "WHERE miner_settings.poolid = @poolid AND miner_settings.address = @address";
